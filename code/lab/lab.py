...
def si_model_rumor_spreading(source, adjancency, N):
    ...
    infected_nodes = [];               #contains original indices
    ...
    # n=0
    infected_nodes.append(source) #set infected_nodes[0] = v*
    susceptible_nodes = temp[source]      #get neighbours
#    susceptible_indices = [0] * len(susceptible_nodes) not used
    for k in susceptible_nodes:
		if(k==source	):
			susceptible_nodes.remove(k)
    ...
    for i in range(1,N):
    	...
    	random_index = randrange(len(susceptible_nodes))
        cur_inf = susceptible_nodes[random_index]   #select v* neigbor randomly 
        infected_nodes.append(cur_inf)                   #put it in infected_nodes
           #index is 1 less
           
        for j in range(0, len(temp[cur_inf])-1):
        	if temp[cur_inf][j] not in infected_nodes:
        		susceptible_nodes.append(temp[cur_inf][j])
        '''		
        ...
        '''
        for k in range(len(susceptible_nodes)-1, -1, -1):
 		if susceptible_nodes[k] == cur_inf:
 			del susceptible_nodes[k]
...
def rumor_centrality(who_infected):
    root_node = 5 # can use any arbitrary index for the root node
    ...
    return highest #random.choice(highest)
'''
...
adjacency[7] = [6]
'''
root_node = 8 # can use any arbitrary index for the root node
up_messages_t =	 [1]*len(who_infected) 
up_messages_p = [1]*len(who_infected)
...
#print rumor_centrality(who_infected)
#print infctn_pattern[6]
#print infctn_pattern[8], infctn_pattern[9]
